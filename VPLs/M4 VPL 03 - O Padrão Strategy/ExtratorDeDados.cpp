...
class Num2Sqrt: public Mapeador<NUM_TYPE, NUM_TYPE> {
  NUM_TYPE transformaDado(NUM_TYPE& d) const override {
    ...
    // Não faz sentido retornar NUM_TYPE. sqrt(2) é float/double, enquanto 2  é int.
    return sqrt(d);
  }
};
...
class FiltroStringNatural: public Filtro<std::string> {
  bool dadoValido(std::string& d) const override {
    ...
    ss << d;
    ss >> natural;
    if (to_string(natural).compare(d) == 0) {
      return true;
    }
    return false;
  }
...
void convert2int(std::vector<std::string>& in, std::vector<int>& out) {
  ...
  Mapeador<string, int>* mapeador = new String2Int();
  for (auto elem : in) {
    if (filtro->dadoValido(elem))
    {
      out.push_back(mapeador->transformaDado(elem));
    }
  }
}
